...
const SAMPLE_POSTS = [
  { id:1, category:"미담", title:"버스에서 쓰러진 할머니를 구한 고등학생", summary:"지난 주 서울 버스 안에서 갑자기 쓰러진 70대 할머니를 17살 고등학생이 심폐소생술로 살려냈습니다. 학교에서 배운 응급처치가 실제 생명을 구했습니다.", source:"실화 제보", likes:342, time:"오늘", comments:[{id:1,author:"따뜻한봄",text:"이런 청년이 있다니 정말 감동이에요.",time:"2시간 전"},{id:2,author:"희망씨앗",text:"학교 교육이 이렇게 빛을 발하는군요!",time:"1시간 전"}]},
  { id:2, category:"동물", title:"10년 동안 할아버지 곁을 지킨 강아지", summary:"치매를 앓던 85세 할아버지의 마지막 순간까지 병원 침대 옆에서 자리를 지킨 반려견 '달이'의 이야기가 많은 이들의 눈물을 자아내고 있습니다.", source:"독자 제보", likes:578, time:"어제", comments:[{id:1,author:"달빛",text:"달이처럼 순수한 사랑이 세상에서 제일 아름다워요.",time:"3시간 전"}]},
  { id:3, category:"나눔", title:"매일 새벽 독거노인 도시락 배달하는 30대 직장인", summary:"5년째 매일 새벽 5시에 일어나 혼자 사는 어르신 12명에게 손수 만든 도시락을 배달하는 회사원 김모씨.", source:"뉴스 기사", likes:891, time:"2일 전", comments:[]},
  { id:4, category:"어린이", title:"용돈 모아 동네 꽃 화분 심은 초등학생 남매", summary:"6살, 8살 남매가 6개월간 모은 용돈 3만 원으로 회색빛 동네 골목에 알록달록 꽃 화분을 심었습니다.", source:"독자 제보", likes:463, time:"3일 전", comments:[{id:1,author:"꽃밭지기",text:"아이들이 어른보다 더 어른스럽네요.",time:"5시간 전"}]},
  { id:5, category:"사랑", title:"50년 해로한 부부, 같은 날 손잡고 세상을 떠나다", summary:"결혼 50주년을 앞두고 나란히 입원한 노부부가 병원 침대에서 손을 꼭 잡은 채 같은 날 평화롭게 눈을 감았습니다.", source:"뉴스 기사", likes:1204, time:"4일 전", comments:[{id:1,author:"별빛",text:"진짜 사랑이 무엇인지 알게 해주는 이야기예요.",time:"1일 전"}]},
];
...
function CardBanner({ category }) {
  ...
  return (
    <div style={{ background: m.bg, height: 130, display:"flex", alignItems:"center", justifyContent:"center", fontSize: 52 }}>
      {m.emoji}
    </div>
  );
}
...
export default function App() {
  ...
  const filtered=posts.filter(p=>(cat==="전체"||p.category===cat)&&(search===""||p.title.includes(search)||p.summary.includes(search)));
  const expandedPost=posts.find(p=>p.id===expanded);

  function toggleLike(id,e){ if(e)e.stopPropagation(); setLiked(prev=>({...prev,[id]:!prev[id]})); setPosts(prev=>prev.map(p=>p.id===id?{...p,likes:p.likes+(liked[id]?-1:1)}:p)); }
  ...
  async function submitPost(){
    ...
    try{
      ...
      setPosts(prev=>[{id:Date.now(),category:newPost.category,title:newPost.title,summary,source:"독자 제보",likes:0,comments:[],time:"방금"},...prev]);
  ...
  const css = `
    ...
    * { box-sizing: border-box; }
    ...
    input:focus, textarea:focus, select:focus { border-color: #E8A87C !important; outline: none; }
  `;
  ...
  return (
    <div style={{background:"#FDF6EE", minHeight:"100vh", fontFamily:"'Gowun Dodum',sans-serif"}}>
      ...
      <div style={{background:"linear-gradient(160deg,#FDE8D0,#FDF0E0 50%,#FCF4E8)", textAlign:"center", padding:"2.5rem 1rem 2rem", borderBottom:"1.5px solid #F2D9BB"}}>
        ...
        <p style={{fontSize:14, color:"#B07850", margin:0}}>세상의 좋은 이야기를 모아요 — 오늘도 누군가 좋은 일을 하고 있어요</p>
      </div>
        ...
        {!expanded ? (<>
            ...
            {filtered.map(post=>(
              <div key={post.id} className="card" onClick={()=>setExpanded(post.id)}>
                ...
                <div style={{padding:"14px 16px"}}>
                  <span style={{fontSize:11, fontWeight:600, padding:"2px 9px", borderRadius:10, background:"#FDE8D0", color:"#9A5530", marginBottom:8, display:"inline-block"}}>
          ...
          <div style={{background:"#FFFAF4", border:"1.5px solid #F2D9BB", borderRadius:18, overflow:"hidden"}}>
            ...
            <div style={{padding:"1.25rem"}}>
              <span style={{fontSize:11, fontWeight:600, padding:"2px 9px", borderRadius:10, background:"#FDE8D0", color:"#9A5530", display:"inline-block", marginBottom:10}}>
