...
const CAT_META = {
  "미담": { emoji:"🌟", bg:"linear-gradient(135deg,#FDE8C8,#FDDBA0)" },
  "사랑": { emoji:"💕", bg:"linear-gradient(135deg,#FDD8D8,#FAB8C0)" },
  "동물": { emoji:"🐾", bg:"linear-gradient(135deg,#D8EDD8,#B8D8B0)" },
  "어린이": { emoji:"🧒", bg:"linear-gradient(135deg,#D8E8F8,#B8D0F0)" },
  "이웃": { emoji:"🏘️", bg:"linear-gradient(135deg,#F0E0C8,#E8D0A8)" },
  "나눔": { emoji:"🤝", bg:"linear-gradient(135deg,#E0D8F8,#C8C0F0)" },
};

const SAMPLE_POSTS = [
  { id:1, category:"미담", title:"버스에서 쓰러진 할머니를 구한 고등학생", summary:"지난 주 서울 버스 안에서 갑자기 쓰러진 70대 할머니를 17살 고등학생이 심폐소생술로 살려냈습니다.", source:"실화 제보", likes:342, time:"오늘", isNews:false, comments:[{id:1,author:"따뜻한봄",text:"이런 청년이 있다니 정말 감동이에요.",time:"2시간 전"}]},
  { id:2, category:"동물", title:"10년 동안 할아버지 곁을 지킨 강아지", summary:"치매를 앓던 85세 할아버지의 마지막 순간까지 병원 침대 옆에서 자리를 지킨 반려견 '달이'의 이야기입니다.", source:"독자 제보", likes:578, time:"어제", isNews:false, comments:[{id:1,author:"달빛",text:"달이처럼 순수한 사랑이 세상에서 제일 아름다워요.",time:"3시간 전"}]},
...
export default function App() {
  ...
  async function fetchNaverNews() {
    ...
    try {
      ...
      if (data && data.items && Array.isArray(data.items)) {
        ...
        setPosts(prev => {
          const existingTitles = new Set(prev.filter(p => p.isNews).map(p => p.title));
          const filtered = newItems.filter(p => !existingTitles.has(p.title));
  ...
  function getCategoryFromTitle(title) {
    if (title.includes("동물") || title.includes("강아지") || title.includes("고양이") || title.includes("유기견")) return "동물";
    if (title.includes("아이") || title.includes("어린이") || title.includes("학생") || title.includes("초등")) return "어린이";
    if (title.includes("사랑") || title.includes("부부") || title.includes("가족") || title.includes("노부부")) return "사랑";
    if (title.includes("나눔") || title.includes("기부") || title.includes("봉사") || title.includes("무료")) return "나눔";
    if (title.includes("이웃") || title.includes("마을") || title.includes("동네") || title.includes("지역")) return "이웃";
    return "미담";
  }
  ...
  function submitComment() {
    ...
    if (hasBadWord(comment)) { setCommentErr("따뜻한 세상에는 따뜻한 말만 담겨요 🌸"); return; }
